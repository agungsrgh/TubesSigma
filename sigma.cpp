...
void displayRumahDalamBlok(adrBlok p){
    ...
    adrRumah q = p->firstRumah;
    if(q == nullptr){
        cout << "Belum ada rumah di blok ini\n";
        return;
    }

    while(q != nullptr){
        cout << "- " << q->info.nomorRumah
             << " | Pemilik: " << q->info.namaPemilik
             << " | Status: " << q->info.statusKepemilikan
             << " | Luas: " << q->info.luasTanah
             << " | Penghuni: " << q->info.jumlahPenghuni
             << endl;
        q = q->next;
    }
}
...
// inisialisasi beberapa data dummy (blok AA, BB, BC)
void initDataDummy(listBlok &L){
    ...
    // --- Blok BC ---
    b = createElementBlok("BC","Serasi");
    tambahBlok(L, b);
    r = createElementRumah("BC1","Tono","Lunas",200,5); tambahRumah(b, r);
}
